...
## Overview
Classify test results from log text (NUnit, PHPUnit, or manual test notes) into 5 statuses. Output feeds into `@qa-test-scorer`.

## When to Use
- Classify results from NUnit/PHPUnit log output
- Classify manual test notes
- Identify flaky tests (inconsistent pass/fail)
- **Do NOT use when:** need to write tests → use `@qa-testcase-generator` or `@automated-unit-testing`
...
## Related Skills
- `@qa-doc-parser` — Parse documents/code into feature plans
- `@qa-testcase-generator` — Generate TCs from feature plans
- `@qa-test-scorer` — Score TCs after verification
