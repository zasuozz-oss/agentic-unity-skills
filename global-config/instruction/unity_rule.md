...
**Choose the cheapest model that can do the subtask:**
- **Haiku** — every tool and param is fully specified by you and the expected output is known (pure execute / read / grep): run a given `batch_execute`, `read_console` for errors only, poll `editor/state`, parse `results.xml`.
- **Sonnet** — the subagent must locate or adapt: find the right GameObject/component, adjust a payload that varies by Unity version, or triage ambiguous console output.
- **Opus / main agent** — never runs MCP; reserved for architecture, root-cause reasoning, and writing the dispatch spec.

Tie-breaker: you wrote every param → Haiku; the subagent must go find something → Sonnet. The subagent returns a distilled result (PASS/FAIL, deduped errors, the requested values) — never raw dumps or base64 images.
